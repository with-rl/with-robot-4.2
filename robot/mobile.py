    ...
    def is_area_safe(self, grid_x, grid_y):
        ...
        for rdx in range(-self.robot_radius_cells, self.robot_radius_cells + 1):
            for rdy in range(-self.robot_radius_cells, self.robot_radius_cells + 1):
                ...
                if not (0 <= check_x < self.width and 0 <= check_y < self.height and self.map[check_x, check_y] == 0):
                    return False
        return True
...
class MobileRobot:
    ...
    def move_to(self, target_name, occupancy_map, display_map=False):
        ...
        if distance_to_target < 0.5:
            trajectory = BaseTrajectory(base_pos, base_pos, base_theta, theta_z, duration=1.0)
        ...
        radius = 0.5
        for angle_deg in np.arange(0, 360, 15):
            angle_rad = np.deg2rad(angle_deg)
            test_goal_pos = target_pos[:2] + radius * np.array([np.cos(angle_rad), np.sin(angle_rad)])
        ...
        if path is None:
            direction = target_pos - base_pos
            fallback_goal = target_pos[:2] - 0.5 * (direction[:2] / np.linalg.norm(direction[:2]))
            fallback_distance = np.linalg.norm(fallback_goal - base_pos)
            fallback_duration = max(0.2, fallback_distance / 5.0)
            trajectory = BaseTrajectory(base_pos, fallback_goal, base_theta, base_theta, duration=fallback_duration)
            self.movement_queue.append(trajectory)
            return
        ...
        self._create_path_trajectories(path, base_pos, base_theta, goal_pos, pathfinder, occupancy_map, target_pos)
    ...
    def _display_map(self, occupancy_map, pathfinder, path):
        ...
        for y in range(h):
            for x in range(w):
                grid_image[y*scale+1:(y+1)*scale, x*scale+1:(x+1)*scale] = image_array[y, x]
        ...
        for i in range(len(all_waypoints) - 1):
            ...
            p1_img_unflipped = (p1_grid[0] * scale + scale // 2, p1_grid[1] * scale + scale // 2)
            p2_img_unflipped = (p2_grid[0] * scale + scale // 2, p2_grid[1] * scale + scale // 2)
    ...
    def _create_path_trajectories(self, path, current_pos, start_theta, goal_pos, pathfinder, occupancy_map, target_pos):
        ...
        for i in range(len(all_waypoints) - 1):
            ...
            duration = max(0.2, distance / 5.0)

            trajectory = BaseTrajectory(start_pos, end_pos, current_theta, current_theta, duration=duration)
            self.movement_queue.append(trajectory)

        final_waypoint = np.array(all_waypoints[-1])
        goal_distance = np.linalg.norm(final_waypoint - goal_pos)
        if goal_distance > 0.05:
            duration = max(0.2, goal_distance / 5.0)
            trajectory = BaseTrajectory(final_waypoint, goal_pos, current_theta, current_theta, duration=duration)
            self.movement_queue.append(trajectory)

        # Add final rotation to face the target
        direction = target_pos[:2] - goal_pos
        target_theta = np.arctan2(direction[1], direction[0])
        if abs(target_theta - current_theta) > 0.1:  # Only rotate if significant angle difference
            rotation_trajectory = BaseTrajectory(goal_pos, goal_pos, current_theta, target_theta, duration=1.0)
